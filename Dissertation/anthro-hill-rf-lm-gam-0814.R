...
analyze_rf_lm_gam <- function(data, species_name, response_variable, output_path) {
  ...
  formula <- as.formula(paste(response_variable, "~ grav_NC + pop_count + num_ports + reef_value + sediment + nutrient"))
  ...
  # 将随机森林的结果导出
  sink(file.path(output_path, paste("anthro_", "rf_", species_name, "_summary_", response_variable, ".txt", sep = "")))
  ...
  # 保存图像
  ggsave(filename = file.path(output_path, paste("anthro_", "rf_", species_name, "_", response_variable, ".pdf", sep = "")), plot = p)
  ...
  if (length(final_plots) > 0) {
    pdf(file.path(output_path, paste("Hill_Diversity_vs_anthro_SingleVar_LM_", response_variable, "_", species_name, ".pdf", sep = "")), height = 6, width = 10)
  ...
  if (length(significant_vars) > 0) {
    ...
    # 打印lm的结果
    sink(file.path(output_path, paste("anthro_", "lm_", species_name, "_summary_", response_variable, ".txt", sep = "")))
  ...
  if (length(insignificant_vars) == 0) {
    ...
    if (length(valid_results) > 0) {
      ...
      # 打印并保存GAM模型结果
      sink(file.path(output_path, paste("anthro_", "gam_", species_name, "_summary_", response_variable, ".txt", sep = "")))
  ...
  if (!is.null(gam_model) && length(remaining_vars) > 0) {
    ...
    if (length(gam_plots) > 0) {
      pdf(file.path(output_path, paste("Hill_Diversity_vs_anthro_GAM_", response_variable, "_", species_name, ".pdf", sep = "")), height = 6, width = 12)
...
# 函数所需参数
species_list = list(Porifera = Combined_Porifera_anthro, 
                    Rhodophyta = Combined_Rhodophyta_anthro)
...
analyze_rf_lm_gam <- function(data, species_name, response_variable, output_path) {
  ...
  formula <- as.formula(paste(response_variable, "~ cumul_score"))
  ...
  # 将随机森林的结果导出
  sink(file.path(output_path, paste("anthro_", "_cumulscore_", "rf_", species_name, "_summary_", response_variable, ".txt", sep = "")))
  print("Randomforest Results:")
  print(importance_df)
  sink()
  ...
  # 保存图像
  ggsave(filename = file.path(output_path, paste("anthro_", "_cumulscore_", "rf_", species_name, "_", response_variable, ".pdf", sep = "")), plot = p)
  ...
  # 将单变量可视化图输出到PDF
  if (length(final_plots) > 0) {
    pdf(file.path(output_path, paste("Hill_Diversity_vs_anthro_SingleVar_LM_cumul_score_", response_variable, "_", species_name, ".pdf", sep = "")), height = 6, width = 10)
    do.call(grid.arrange, c(final_plots, ncol = 2))
    dev.off()
  } else {
    cat("No plots generated for single variable linear regressions.\n")
  ...
  if (length(significant_vars) > 0) {
    ...
    # 打印lm的结果
    sink(file.path(output_path, paste("anthro_", "_cumul_score_", "lm_", species_name, "_summary_", response_variable, ".txt", sep = "")))
    print("Initial Model Summary:")
    print(summary(original_lm_model))
    print("Model After Stepwise Selection:")
    print(summary(step_model))
  ...
  if (length(insignificant_vars) == 0) {
    ...
    if (length(valid_results) > 0) {
      ...
      # 打印并保存GAM模型结果
      sink(file.path(output_path, paste("anthro_", "_cumul_score_", "gam_", species_name, "_summary_", response_variable, ".txt", sep = "")))
      print("GAM best k of variables:")
      print(remaining_vars)
      print("GAM Model Summary:")
      print(summary(gam_model))
  ...
  if (!is.null(gam_model) && length(remaining_vars) > 0) {
    ...
    if (length(gam_plots) > 0) {
      pdf(file.path(output_path, paste("Hill_Diversity_vs_anthro_GAM_cumul_score", response_variable, "_", species_name, ".pdf", sep = "")), height = 6, width = 12)
      do.call(grid.arrange, c(gam_plots, ncol = 3))
      dev.off()
    } else {
      cat("没有有效的绘图对象，跳过GAM图生成。\n")
